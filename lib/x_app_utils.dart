...
class XAppUtils with WidgetsBindingObserver {
  factory XAppUtils() => _instance;
  XAppUtils._();
  static final XAppUtils _instance = XAppUtils._();
  static const MethodChannel _channel = MethodChannel('x_app_utils');
  ...
  static XAppUtils get instance => _instance;
  ...
  Future<XAppUtils> get ready async {
    await init();
    return this;
  }
  ...
  Future<void> writeLocalId(
    ...
    await _ensureInitialized();
    final values = await _call('writeLocalId', <String, dynamic>{
      'slot': slot.wireName,
      'value': value,
    });
    _ensureLocalIdOperationSucceeded('writeLocalId', values);
    _merge(values);
  ...
  Future<void> deleteLocalId({
    ...
    final values = await _call(
      'deleteLocalId',
      <String, dynamic>{'slot': slot.wireName},
    );
    _ensureLocalIdOperationSucceeded('deleteLocalId', values);
  ...
  Future<String> resetLocalId({
    ...
    _ensureLocalIdOperationSucceeded('resetLocalId', values);
    _merge(values);
    _ensureLocalIdsInMemory();
    return _s(_localIdDataKey(slot));
  }
  ...
  Future<void> deleteAllLocalIds() async {
    await _ensureInitialized();
    final values = await _call('deleteAllLocalIds');
    _ensureLocalIdOperationSucceeded('deleteAllLocalIds', values);
    _data = <String, dynamic>{..._data}
      ..remove('primaryLocalId')
      ..remove('secondaryLocalId');
  }
  ...
  Future<Map<LocalIdSlot, String>> resetAllLocalIds() async {
    await _ensureInitialized();
    final values = await _call('resetAllLocalIds');
    _ensureLocalIdOperationSucceeded('resetAllLocalIds', values);
    _merge(values);
    _ensureLocalIdsInMemory();
    return <LocalIdSlot, String>{
      LocalIdSlot.primary: primaryLocalId,
  ...
  void _ensureLocalIdOperationSucceeded(
    String operation,
    Map<String, dynamic> values,
  ) {
    if (values['success'] != true) {
      throw StateError('$operation failed to persist the local ID.');
    }
  }
