...
public final class AppInfoHelperPlugin: NSObject, FlutterPlugin {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    case "deleteLocalId": localIds.delete(slot(call), options: options); result(["localIdsPersisted": true])
    ...
    case "deleteAllLocalIds": localIds.deleteAll(options); result(["localIdsPersisted": true])
...
private struct LocalIdResult {
  ...
  let persisted: Bool
  var map: [String: Any] { ["primaryLocalId": primary, "secondaryLocalId": secondary, "localIdsPersisted": persisted] }
}
...
private final class SecureLocalIdStorage {
  ...
  func delete(_ slot: LocalIdSlot, options: LocalIdOptions) {
    keys(slot, options: options).forEach { deleteKey($0) }
  }
  func deleteAll(_ options: LocalIdOptions) {
    delete(.primary, options: options)
    delete(.secondary, options: options)
  }
